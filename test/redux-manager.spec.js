...
describe("Store set up", () => {
  ...
  describe("The smart state", () => {
    ...
    it("should have the initial values as the ones passed in", () => {
      ...
      expect(testStore.testArrayofObject.path).toBe(
        "testStore.testArrayofObject"
      );
    ...
    it("should have get property that works as a selector", () => {
      ...
      expect(testStore.testString.get()).toEqual("wuluta_abulala");
    ...
    it("should have a set property and its type should be a function", () => {
      ReduxManager.dispatch = jest
        .spyOn(ReduxManager, "dispatch")
        .mockImplementation(action => {
          // console.log({action})
        });
      testStore.testNumber.set(22);
      expect(ReduxManager.dispatch).toBeCalledWith({
        type: "testStore.testNumber.set",
        value: 22
      });
      ...
      expect(ReduxManager.dispatch).toBeCalledWith({
        type: "testStore.testBoolean.set",
        value: true
      });
      ...
      expect(ReduxManager.dispatch).toBeCalledWith({
        type: "testStore.testObject.update",
        value: { part5: "str5" }
      });
      ...
      expect(ReduxManager.dispatch).toBeCalledWith(
        expect.objectContaining({ type: "testStore.testArray.deleteItems" })
      );
      ...
      expect(typeof testStore.testArray.updateItems).toBe("function");
      testStore.testArray.updateItems(item => item.newObj2 === "orion", {
        newObj2: "nonion"
      });
      expect(ReduxManager.dispatch).toBeCalledWith(
        expect.objectContaining({
    ...
    it("should have a reducers property and its functionality", () => {
      ...
      action = {
        type: "testStore.testArray.deleteItems",
        query: item => item === "orion"
      };
      newState = rootReducer(oldState, action);
      ...
      action = {
        type: "testStore.testArrayofObject.deleteItems",
        query: item => item.obj2 === "onion"
      };
      newState = rootReducer(oldState, action);
      expect(newState.testArrayofObject).toEqual([
      ...
      action = {
        type: "testStore.testArrayofObject.addItem",
        item: { obj5: "norion" }
      };
      newState = rootReducer(oldState, action);
      expect(newState.testArrayofObject).toEqual([
    ...
    describe("The nested smart state", () => {
      ...
      it("should have a get property and its type should be a function", () => {
        ...
        expect(testStore.smartStateNested.testArrayNested.get()).toEqual([
          "roion",
          "noion",
          "orion",
          "onion"
      ...
      it("should have a set property and its type should be a function", () => {
        ...
        let action = {
          type: "testStore.smartStateNested.testArrayNested.set",
          value: 102
        };
        ReduxManager.dispatch = jest
          .spyOn(ReduxManager, "dispatch")
